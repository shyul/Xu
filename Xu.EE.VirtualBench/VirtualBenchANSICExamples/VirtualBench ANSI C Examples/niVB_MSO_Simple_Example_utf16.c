/******************************************************************************
*
* niVB_MSO_Simple_Example_utf16.c
*
* Description:
...
*    1. Modify the L"deviceName" parameter to be the name of the device that is
*       being used.  By default it is set to L"virtualbench".
...
static niVB_MSO_SamplingMode querySamplingMode(niVB_MSO_InstrumentHandle msoHandle)
{
   ...
   {
      wprintf(L"Error querying timing.\n");
   }
   return samplingMode;
}
...
static size_t queryNumChannels(niVB_LibraryHandle libHandle, niVB_MSO_InstrumentHandle msoHandle)
{
   ...
   wchar_t *enabledChannels = NULL;
   bool failed = true;

   if (!niVB_Status_Failed(niVB_MSO_QueryEnabledAnalogChannelsW(msoHandle, NULL, 0, &enabledChannelsSize)))
   {
      enabledChannels = (wchar_t*)malloc(enabledChannelsSize * sizeof(wchar_t));
      if (!niVB_Status_Failed(niVB_MSO_QueryEnabledAnalogChannelsW(msoHandle, enabledChannels, enabledChannelsSize, NULL)))
      {
         if (!niVB_Status_Failed(niVB_CollapseChannelStringW(libHandle, enabledChannels, NULL, 0, NULL, &numChannels)))
         {
            failed = false;
         }
      }
   ...
   if (failed)
   {
      wprintf(L"Error querying number of channels!\n");
   }
...
int __cdecl main(void)
{
   ...
   if (status == niVB_Status_Success)
   {
      ...
      if (analogDataSize != 0)  wprintf(L"Analog t0 is %g seconds before trigger\n", getTimestampDifference(triggerTimestamp, analogT0));
      if (digitalDataSize != 0) wprintf(L"Digital t0 is %g seconds before trigger\n", getTimestampDifference(triggerTimestamp, digitalT0));
