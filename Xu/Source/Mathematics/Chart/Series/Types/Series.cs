...
namespace Xu.Chart
{
    public abstract class Series : IItem, IEquatable<Series>, IDependable
    {
        ...
        public static IEnumerable<TagInfo> GetTags(ITable table, IEnumerable<ObjectColumn> columns, int index) => columns.Select(n => table[index, n]).Where(n => n is TagInfo).Select(n => (TagInfo)n);
